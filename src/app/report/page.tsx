...
import { MapPin, Upload, CheckCircle, Loader, AlertTriangle } from 'lucide-react';
...
export default function ReportPage() {
  ...
  useEffect(() => {
    if (!geminiApiKey || !googleMapsApiKey) {
      ...
      toast.error("Application is not configured correctly. Missing API keys.");
    }
  }, []);
  ...
  const handleVerify = async () => {
    if (!file || !geminiApiKey) {
      console.log("handleVerify aborted - missing file or API key:", { file: !!file, geminiApiKey: !!geminiApiKey });
      return;
    }
    ...
    try {
      console.log("Starting AI verification with gemini-2.0-flash...");
      ...
      const prompt = `
You are an expert in waste management and environmental science.
Analyze this image of waste and provide a detailed assessment.
Provide consistent and accurate estimations based on visual cues. If you see the same image multiple times, your assessment must be identical.

Respond ONLY with a valid JSON object.
Do NOT include markdown formatting like backticks or "json" labels.
Do NOT include any explanations or extra text.

The JSON should have these exact keys:
1. "wasteType": Identify the primary type of waste (e.g., Plastic, Paper, Metal, Organic, Electronic, Glass, Mixed).
2. "reasoning": Briefly describe the visual cues you used for estimation (e.g., "I see about 10 large plastic bags", "Typical 60L bag is half full"). This helps maintain consistency.
3. "quantity": Estimate the amount of waste. Be as specific as possible. 
   - Reference scales: A standard large trash bag is ~60 Liters. A standard water bottle is 0.5 Liters.
   - If it's count-based (e.g., bottles, cans), provide a number (e.g., "5 plastic bottles").
   - If it's volume-based, use liters or bags (e.g., "Approximately 20 liters", "Half-full 60L trash bag").
   - If it's weight-based, use kg (e.g., "About 1.5 kg").
4. "confidence": A number between 0 and 1 representing your certainty in this assessment.

Format strictly like this:
{
  "wasteType": "type of waste",
  "reasoning": "visual cues used",
...
`;
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      toast.success(`Report submitted successfully! You've earned points.`);
    } catch (error) {
          ...
          <div className="bg-red-50 border-l-4 border-red-400 p-4 mb-8 rounded-r-xl">
            <div className="flex items-center">
              ...
              <div>
                ...
                <p className="mt-2 text-sm text-red-700">Could not analyze the image. Please try a different image or try again.</p>
              </div>
            </div>
          </div>
        )}
